...
| FPT-REASONING-POC | 2026-09-12 | simulate_v4 synthetic workbook; 10 business groups; research ablation A/B/C/D | Deterministic profile/rule contract; LLM repeats configurable; no gold metric asserted | Pipeline smoke verified on supplied workbook: 10 profiles, 10 rule results; AI metrics `not_evaluable` until reviewed gold responses exist | [FPT pipeline](../pipelines/fpt_credit_reasoning_poc.md); ignored local `outputs/research/fpt_reasoning_poc/` | Implemented research candidate; no production decision claim |
